...
template <typename PointType>
void KD_TREE<PointType>::Build(PointType *point_cloud, size_t size)
{
    if (Root_Node != nullptr)
    {
        delete_tree_nodes(&Root_Node);
...
{
    std::lock_guard<std::mutex> lock(ikd_mutex);
...
template <typename PointType>
void KD_TREE<PointType>::BuildTree(KD_TREE_NODE **root, int l, int r, PointType *cloud,
                                   size_t size)
{
    PointVector Storage = PointVector(cloud, cloud + size);
    BuildTree(root, l, r, Storage);
}
...
template <typename PointType>
void KD_TREE<PointType>::consistent()
{
    if (Root_Node == nullptr)
    {
    ...
    // consistent(Root_Node);
}

/**
 * @brief Iterates the entire tree and assures that the nodes that to be pushed down are pushed
 * down.
 */
template <typename PointType>
void KD_TREE<PointType>::consistent(KD_TREE_NODE *root)
{
    if (root == nullptr)
    {
        // std::cout << "root null" << std::endl;
    ...
    // std::cout << "Checking node: " << root << std::endl;

    if (root->need_push_down_to_left || root->need_push_down_to_right)
    {
        pthread_mutex_lock(&(root->push_down_mutex_lock));
        // std::cout << "push down node: " << root << std::endl;
        Push_Down(root);
        pthread_mutex_unlock(&(root->push_down_mutex_lock));
        if (root->need_push_down_to_left || root->need_push_down_to_right)
        {
            // std::cout << "push down failed" << std::endl;
        }
        else
        {
            // std::cout << "push down success" << std::endl;
        }
    }
    else
    {
        // std::cout << "no need to push down" << std::endl;
    }

    // std::cout << "push left son: " << root->left_son_ptr << std::endl;
    consistent(root->left_son_ptr);
    // std::cout << "push right son: " << root->left_son_ptr << std::endl;
    consistent(root->right_son_ptr);

    //  std::cout << "pushed tree" << std::endl;
